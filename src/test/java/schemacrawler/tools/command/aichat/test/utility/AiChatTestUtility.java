...
@UtilityMarker
public class AiChatTestUtility {

  private static class EmbeddingResult extends Embedding<EmbeddingFloat> {

    private final Usage usage;

    public EmbeddingResult(final Usage usage) {
      this.usage = usage;
    }

    @Override
    public List<EmbeddingFloat> getData() {
      return Collections.singletonList(new EmbeddingFloat() {});
    }
  ...
  public static SimpleOpenAI setUpMockAiService(final List<Double> expectedEmbedding) {
    try {
      ...
      final EmbeddingResult embeddingResult = new EmbeddingResult(usage);
