...
public class JsonSchemaToolUtilityTest {
	...
	public void functionParameters() {
		...
		List<Class<? extends FunctionParameters>> functionParameters = List.of(DescribeTablesFunctionParameters.class,
				DescribeRoutinesFunctionParameters.class, LintFunctionParameters.class, ListFunctionParameters.class,
				ListAcrossTablesFunctionParameters.class, DiagramFunctionParameters.class,
				TableSampleFunctionParameters.class);

		final TestWriter testout = new TestWriter();
		try (final TestWriter out = testout) {
			for (final Class<?> parametersClass : functionParameters) {
				SortedMap<String, JsonNode> parametersSchemaMap = new TreeMap<>(
						ToolUtility.extractParametersSchemaMap(parametersClass));
		...
		assertThat(outputOf(testout), hasSameContentAs(classpathResource("parameters-json-schemas.txt")));
	}
}
